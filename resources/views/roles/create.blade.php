...
<div class="card card-custom">
    ...
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center w-100 mb-4">
            <h6 class="card-title mb-0">Add a new Role</h6>
        </div>
        <form class="cmxform" data-form-id="role_add_form" id="role_add_form" method="POST" action="{{ route('role.create.action') }}">
            @csrf
            <div class="form-group row">
                <label for="role_name" class="col-sm-3 col-form-label">Role Name</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control @error('udp.role_name') is-invalid @enderror" id="role_name" name="udp[role_name]" placeholder="Manager/ Company Manager" value="{{ old('udp.role_name') }}" onblur="generateSlug(this)" required>
                    @error('udp.role_name')
            ...
            <div class="form-group row">
                <label for="role_slug" class="col-sm-3 col-form-label">Slug</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control  @error('udp.role_slug') is-invalid @enderror" id="role_slug" name="udp[role_slug]" placeholder="manager/ company_manager" value="{{ old('udp.role_slug') }}" required>
                    @error('udp.role_slug')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                            ...
                            <li class="list-group-item-parent">
                                ...
                                <ul class="fa-ul list-group collapse show" aria-expanded="true">
                                    ...
                                    <li data-value="{{ $permission->permissions_name }}" class="list-group-item-inner child-li" data-group="1" data-relate="{{ strtolower($modelEach->model) }}">
                                        <div class="form-check form-check-inline">
                                            <label class="form-check-label">
                                                <input data-child="{{ strtolower($modelEach->model) }}" data-treeview="true" data-group="1" type="checkbox" name="udp[role_permission][]" class="form-check-input tw-control" data-value="{{ $permission->permissions_name }}" value="{{ $permission->permissions_name }}">{{ $permission->display_name }}
                                            </label>
                                        </div>
                                    </li>
                                    @endforeach
