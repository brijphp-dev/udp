    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap p-8 bgi-size-cover bgi-no-repeat rounded-top" style="background-image: url('{{ asset('media/misc/bg-1.jpg') }}')">
        <div class="d-flex align-items-center mr-2">
            ...
            <div class="symbol bg-white-o-15 mr-3">
                <span class="symbol-label text-success font-weight-bold font-size-h4">{{ $userDetail->first_name[1] ?? "S" }}</span>
            </div>

            {{-- Text --}}
            <div class="text-white m-0 flex-grow-1 mr-3 font-size-h5">{{ $userDetail->first_name ?? "Super" }} {{ $userDetail->last_name ?? "User" }}</div>
        </div>
    </div>
@endif
...
<div class="navi navi-spacer-x-0 pt-5">
    ...
    <div class="navi-footer  px-8 py-5">
        <a href="{{ route('admin.user.logout') }}" class="btn btn-light-primary font-weight-bold" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">Sign Out</a>
