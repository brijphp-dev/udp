...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['needPermission'])) {
                $permissionArray = [];
                foreach ($item['needPermission'] as $needPermission) {
                    $permissionArray[] = route($needPermission);
                }
                //dd($page, app('router')->getRoutes()->getByName('admin.user.edit.form')->uri);
            ...
            if (isset($item['submenu']) && self::isActiveVerMenuItem($item, request()->url())) {
                $item_class .= ' menu-item-open menu-item-here'; // m-menu__item--active
            } elseif (self::isActiveVerMenuItem($item, request()->url())) {
            ...
            if (isset($item['parent'])) {
                ...
                if (isset($item['page'])) {
                    $url = (isset($item['isRoute'])) ? route($item['page']) : url($item['page']);
                }
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page'])) {
            ...
            return (isset($item['isRoute'])) ? ( ( route($item['page']) == $page ) ? true : false ) : false;
        }
        
        if (isset($item['isActive'])) {
            $returnResponce = false;
            foreach($item['isActive'] as $activesubmenu){
                if( is_active_route($activesubmenu) && !$returnResponce) {
                    $returnResponce = true;
                };
            }
